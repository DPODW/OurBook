...
public class loginCheckApi {
    ...
    public ResponseEntity<String> checkLogin(HttpServletRequest request, CustomUserDetail userDetail){
      HttpSession session = request.getSession(false);
      SessionUser naverMember = (SessionUser) session.getAttribute("NAVER");
      if(naverMember==null && userDetail.getEmail()==null){ //빈 껍데기 userDetail 이 생성되기 때문에, email 을 가져올수 있느냐 라는 조건으로 판별
